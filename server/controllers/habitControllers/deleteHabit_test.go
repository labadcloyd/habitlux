...
func TestDeleteHabit(t *testing.T) {
	...
	app.Delete("/api/habit", func(c *fiber.Ctx) error { DeleteHabit(c, db); return nil })
	...
	tests := []struct {
	...
	}{
		{
			...
			cookie:       cookie,
			expectedCode: 200,
			expectedBody: "{\"message\":\"Successfully deleted habbit\"}",
			setupFunc: func() {
				db.QueryRow(`
					INSERT INTO
	...
	for _, test := range tests {
		...
		// setting the request
		req := httptest.NewRequest("DELETE", "/api/habit", bytes.NewBuffer(payload))
